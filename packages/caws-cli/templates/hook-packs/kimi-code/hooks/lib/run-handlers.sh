...
# Kimi Code handler-dispatch loop (kimi-code override).
#
# This file overrides the shared lib/run-handlers.sh for the kimi-code surface.
...
# exists (i.e. .kimi-code/hooks/lib/run-handlers.sh is present in the consumer
# repo).
#
# Kimi-specific differences from the shared baseline (two changes):
#   1. _rh_stdout_priority recognizes "deny" as priority-3 (same as "block"),
#      causing an immediate short-circuit and return 2. The kimi emit override
#      maps ask-level escalations to permissionDecision "deny" (Kimi has no
#      blocking "ask" — verified live on 0.31.1); without this arm a deny
#      envelope would rank as ordinary context (priority 1) and could be
#      displaced by a later handler's additionalContext, silently allowing a
#      guard's escalation. Exit 2 + the mirrored stderr reason is exactly Kimi's
#      documented block contract. Codex adapter precedent.
#   2. A non-zero aggregate exit is promoted to the blocking exit 2 (see the
#      tail of run_handlers). Kimi has no non-blocking error tier — a
#      PreToolUse hook exiting 1 is NOT enforced (verified live on 0.31.1:
#      the tool executes). The shared baseline treats non-2 non-zero exits as
#      advisories and returns the max code; on this surface that fails OPEN
#      for guards whose refusal path exits 1 (protected-paths.sh's
#      fail-closed arm). Promotion preserves the refusal; the handler's stderr
#      was already re-emitted with its prefix, and stderr is the channel Kimi
#      surfaces as the block reason on exit 2.
#
...
_rh_stdout_priority() {
  ...
  case "$decision" in
    # Kimi-specific: "deny" is priority-3 (immediate short-circuit, return 2)
    # alongside "block" — the kimi emit override maps ask-level escalations
    # to deny, and deny must block, not rank as ordinary context.
    deny) printf '3\n' ;;
    block) printf '3\n' ;;
    ask) printf '2\n' ;;
    *) printf '1\n' ;;
...
run_handlers() {
  ...
  # Kimi-specific (header difference 2): no non-blocking error tier exists on
  # this surface — exit 1 is not enforced by Kimi. Promote any non-zero
  # aggregate to the blocking exit 2 so guard refusals cannot fail open.
